...
extension MigrationManager {
    ...
    private func migrateControlItems0_10_1() -> MigrationResult {
        ...
        if needsResetPreferredPositions {
            ...
            let alert = NSAlert()
            alert.messageText = Localization.string("Due to a bug in the 0.10.0 release, the data for Ice's menu bar items was corrupted and their positions had to be reset.")
            alert.informativeText = Localization.string("Our sincerest apologies for the inconvenience.")
