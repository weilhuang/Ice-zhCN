...
final class LayoutBarItemView: NSView {
    ...
    func provideAlertForDisabledItem() -> NSAlert {
        let alert = NSAlert()
        alert.messageText = Localization.string("Menu bar item is not movable.")
        alert.informativeText = Localization.string("macOS prohibits \"\(item.displayName)\" from being moved.")
        return alert
    }
    ...
    func provideAlertForUnresponsiveItem() -> NSAlert {
        ...
        alert.informativeText = Localization.string("\(item.displayName) is unresponsive. Until it is restarted, it cannot be moved. Movement of other menu bar items may also be affected until this is resolved.")
        return alert
    }
